<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span>/</span>
        <span class="text-gray-700 font-bold"><?= htmlspecialchars($currentCategory['name']) ?></span>
        <?php else: ?>
        <span class="text-gray-700 font-bold">ສິນຄ້າທັງໝົດ</span>
        <?php endif; ?>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <?php if (!empty($search)): ?>
            <h1 class="text-2xl md:text-3xl font-black text-gray-800">ຜົນການຄົ້ນຫາ: "<?= htmlspecialchars($search) ?>"</h1>
            <p class="text-sm text-gray-500 mt-1">ພົບ <?= (int)$total ?> ລາຍການ</p>
            <?php elseif (!empty($currentCategory)): ?>
            <h1 class="text-2xl md:text-3xl font-black text-gray-800"><?= htmlspecialchars($currentCategory['name']) ?></h1>
            <p class="text-sm text-gray-500 mt-1"><?= (int)$total ?> ລາຍການ</p>
            <?php else: ?>
            <h1 class="text-2xl md:text-3xl font-black text-gray-800">ສິນຄ້າທັງໝົດ</h1>
            <p class="text-sm text-gray-500 mt-1">ພົບ <?= (int)$total ?> ລາຍການ</p>
            <?php endif; ?>
        </div>
        <div class="flex items-center gap-3">
            <select onchange="window.location.href=this.value" class="px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="lg:w-64 flex-shrink-0">
            <div class="bg-white rounded-2xl border border-gray-100 p-5 sticky top-28">
                <h3 class="text-sm font-black text-gray-800 uppercase tracking-wider mb-4">ໝວດສິນຄ້າ</h3>
                <div class="space-y-1">
                    <a href="<?= url('/products') ?><?= !empty($search) ? '?search=' . urlencode($search) : '' ?>" class="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all <?= empty($categoryId) ? 'bg-sky-50 text-sky-600' : 'text-gray-600 hover:bg-gray-50' ?>">
                        <span>ທັງໝົດ</span>
                    </a>
                    ...
                    ?>
                    <a href="<?= $catUrl ?>" class="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all <?= (!empty($categoryId) && $categoryId == $cat['id']) || (!empty($categorySlug) && $categorySlug === $cat['slug']) ? 'bg-sky-50 text-sky-600' : 'text-gray-600 hover:bg-gray-50' ?>">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="text-center py-20">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">ບໍ່ພົບສິນຄ້າ</h3>
                <p class="text-sm text-gray-500 mb-6">ບໍ່ມີສິນຄ້າທີ່ກົງກັບການຄົ້ນຫາຂອງທ່ານ</p>
                ...
                <div class="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-sky-100/30 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <div class="p-4">
                        ...
                        <div class="mt-3 flex items-center gap-2">
                            ...
                            <a href="<?= url('/products/' . htmlspecialchars($product['slug'])) ?>" class="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:border-sky-200 hover:text-sky-600 transition-all">
