<div class="h-full flex flex-col" x-data="posApp()" x-init="init()" x-cloak>
    <div class="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <div class="flex-1 flex flex-col overflow-hidden border-r">
            <div class="p-3 md:p-4 border-b bg-card space-y-3">
            ...
            <div class="flex-1 overflow-y-auto p-3 md:p-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 md:gap-3">
                    <template x-for="product in filteredProducts" :key="product.id">
                        <div @click="addToCart(product)" :class="{'opacity-50 cursor-not-allowed': product.stock <= 0}" class="bg-card rounded-2xl border border-border p-3 hover:shadow-lg hover:border-primary/20 transition-all cursor-pointer active:scale-[0.98]">
                            <div class="h-16 w-full rounded-2xl bg-gradient-to-br from-muted to-muted flex items-center justify-center text-muted-foreground mb-2 overflow-hidden">
                            ...
                            <p class="text-xs font-bold text-foreground truncate" x-text="product.name"></p>
        ...
        <div class="w-full lg:w-96 xl:w-[420px] flex flex-col bg-card">
                    ...
                    <div class="bg-gray-50 rounded-xl p-3 space-y-2">
                        <div class="flex items-start justify-between">
                            <div class="flex-1 min-w-0 mr-2">
                                <p class="text-xs font-bold text-foreground truncate" x-text="item.name"></p>
                                <p class="text-[10px] text-muted-foreground" x-text="formatPrice(item.price) + ' / ' + (item.unit || 'ຊິ້ນ')"></p>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center border rounded-lg bg-card">
                                <button @click="updateQty(index, item.qty - 1)" class="w-8 h-8 rounded-xl bg-gray-50 hover:bg-primary/10 hover:text-primary transition-all flex items-center justify-center text-xs font-bold text-muted-foreground">-</button>
            ...
            <div class="border-t p-3 md:p-4 space-y-3">
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-foreground/70">ລູກຄ້າ</label>
                ...
                <div class="flex items-center justify-between py-2 border-t">
                    <span class="text-xs font-bold text-foreground/85">ລວມທັງໝົດ</span>
