    ...
    <div class="max-w-5xl mx-auto space-y-6 animate-fade-in">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-0.5">ໃບສະເໜີລາຄາ - <?= htmlspecialchars($template['label']) ?></p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-50">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">ສິນຄ້າທີ່ສະເໜີລາຄາ</p>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-100">
                                    ...
                                    <th class="py-3 px-2 font-bold text-gray-500 text-xs uppercase tracking-wider text-center">ຈຳນວນ</th>
                                    <th class="py-3 px-2 font-bold text-gray-500 text-xs uppercase tracking-wider text-center">ຫົວໜ່ວຍ</th>
                                    <th class="py-3 px-2 font-bold text-gray-500 text-xs uppercase tracking-wider text-right">ລາຄາ/ໜ່ວຍ</th>
                                    <th class="py-3 px-2 font-bold text-gray-500 text-xs uppercase tracking-wider text-right">ຈຳນວນເງິນ</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr class="border-b border-gray-50 last:border-0">
                                    ...
                                    <td class="py-3 px-2 text-gray-600 text-center"><?= (float)$item['quantity'] ?></td>
                                    <td class="py-3 px-2 text-gray-600 text-center"><?= htmlspecialchars($item['unit'] ?: '-') ?></td>
                                    <td class="py-3 px-2 text-gray-600 text-right"><?= number_format($item['unit_price'], 0) ?></td>
                                    <td class="py-3 px-2 font-medium text-gray-800 text-right"><?= number_format($item['amount'], 0) ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100 flex flex-col items-end">
                        <div class="w-full max-w-xs space-y-1.5 text-sm">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-500">ອາກອນມູນຄ່າເພີ້ມ (<?= (float)$quotation['tax_percent'] ?>%)</span>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    <h3 class="text-sm font-bold text-gray-800 mb-2">ໝາຍເຫດ</h3>
                    <p class="text-sm text-gray-600 whitespace-pre-line"><?= nl2br(htmlspecialchars($quotation['notes'])) ?></p>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-50">
                        ...
                        <div>
                            <h2 class="text-base font-extrabold text-gray-800">ຂໍ້ມູນໃບສະເໜີລາຄາ</h2>
                        </div>
                    </div>
                    <div class="space-y-3">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ແມ່ແບບ</p>
                            <p class="text-sm font-bold text-gray-800"><?= htmlspecialchars($template['label']) ?></p>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ວັນທີ</p>
                            <p class="text-sm text-gray-800"><?= $quotation['date'] ? date('d/m/Y', strtotime($quotation['date'])) : '-' ?></p>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ເລກອ້າງອີງ</p>
                            <p class="text-sm text-gray-800"><?= htmlspecialchars($quotation['ref_no'] ?: '-') ?></p>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ສະຖານະ</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    ...
                    <p class="text-sm font-bold text-gray-800"><?= htmlspecialchars($quotation['supplier_name'] ?: '-') ?></p>
                    <?php if (!empty($quotation['supplier_contact'])): ?>
                    <p class="text-sm text-gray-500 mt-1"><?= htmlspecialchars($quotation['supplier_contact']) ?></p>
                    <?php endif; ?>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    <div class="flex items-center gap-3 mb-4 pb-4 border-b border-gray-50">
                        ...
                        <div>
                            <h2 class="text-base font-extrabold text-gray-800">ເງື່ອນໄຂ</h2>
                        </div>
                    </div>
                ...
                <p class="text-xs text-gray-400 text-center">ສ້າງເມື່ອ: <?= date('d/m/Y H:i', strtotime($quotation['created_at'])) ?></p>
            </div>
        </div>
    </div>
</div>
