        ...
        <div class="bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow p-6 md:p-8">
            <form method="GET" action="<?= url('/customers') ?>" class="flex flex-col sm:flex-row gap-3 mb-6">
                <div class="flex-1 relative group">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>" placeholder="ຄົ້ນຫາຊື່ ຫຼື ເບີໂທ..." class="w-full pl-9 pr-4 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all bg-card text-sm placeholder:text-gray-300">
                </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-border">
                            ...
                            <th class="py-3 px-2 font-bold text-muted-foreground text-xs uppercase tracking-wider">ຍອດຊື້ທັງໝົດ</th>
                        ...
                        <tr class="border-b border-gray-50 last:border-0">
                            ...
                            <td class="py-3 px-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-medium text-foreground"><?= htmlspecialchars($c['name']) ?></span>
                                </div>
                            </td>
                            ...
                            <td class="py-3 px-2 text-foreground/70"><?= number_format($c['total_purchases'] ?? 0, 0) ?> ກີບ</td>
                            <td class="py-3 px-2 text-foreground/70"><?= htmlspecialchars(date('d/m/Y', strtotime($c['created_at']))) ?></td>
                            <td class="py-3 px-2">
