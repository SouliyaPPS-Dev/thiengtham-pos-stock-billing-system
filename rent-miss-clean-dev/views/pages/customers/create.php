<div class="p-4 md:p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">ເພີ່ມລູກຄ້າ</h1>
            <p class="text-sm text-gray-500">Add new customer</p>
        </div>
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border p-6" onsubmit="var btn=this.querySelector('button[type=submit]'); btn.disabled=true; btn.classList.add('opacity-60','cursor-not-allowed'); btn.innerHTML='<i class=\"fas fa-spinner fa-spin mr-2\"></i> ກຳລັງບັນທຶກ...';">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="id_card_no" value="<?= htmlspecialchars($_POST['id_card_no'] ?? '') ?>"
                    class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="customer_type" class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none">
                    <option value="Walk-in" <?= ($_POST['customer_type'] ?? 'Walk-in') === 'Walk-in' ? 'selected' : '' ?>>Walk-in</option>
                    <option value="Regular" <?= ($_POST['customer_type'] ?? '') === 'Regular' ? 'selected' : '' ?>>Regular</option>
                    <option value="VIP" <?= ($_POST['customer_type'] ?? '') === 'VIP' ? 'selected' : '' ?>>VIP</option>
                    <option value="Corporate" <?= ($_POST['customer_type'] ?? '') === 'Corporate' ? 'selected' : '' ?>>Corporate</option>
                </select>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Address</label>
                <textarea name="address" rows="2" class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none"><?= htmlspecialchars($_POST['address'] ?? '') ?></textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="district" value="<?= htmlspecialchars($_POST['district'] ?? '') ?>"
                    class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none">
            </div>
            <div>
                ...
                <select name="status" class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none">
                    ...
                    <option value="Blacklisted" <?= ($_POST['status'] ?? '') === 'Blacklisted' ? 'selected' : '' ?>>Blacklisted</option>
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="3" class="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-sky-500 outline-none"><?= htmlspecialchars($_POST['notes'] ?? '') ?></textarea>
            </div>
        </div>
        <div class="flex items-center justify-end gap-4 mt-6 pt-6 border-t">
            <a href="/customers" class="px-6 py-2.5 border text-gray-600 rounded-xl font-medium hover:bg-gray-50">
                Cancel
            </a>
